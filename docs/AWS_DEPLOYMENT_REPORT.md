...
- API URL: `https://xmfe23zrnivkzaomrezacvyhve0wpank.lambda-url.us-east-1.on.aws/`
- API health URL: `https://xmfe23zrnivkzaomrezacvyhve0wpank.lambda-url.us-east-1.on.aws/health`
...
Custom domain status: `clipforged.xyz` was purchased through Porkbun and delegated to AWS Route53 by replacing the Porkbun nameservers with the Route53 hosted-zone nameservers. CDK created a DNS-validated ACM certificate in `us-east-1`, attached `clipforged.xyz` and `www.clipforged.xyz` as aliases on the frontend CloudFront distribution, and created Route53 alias records for both hostnames.
...
11. Added the custom domain:
   ...
   This created and validated the ACM certificate, updated CloudFront aliases, created Route53 alias records, updated API CORS origins, and updated S3 upload CORS origins.
...
| Playback URL generation | `200 OK`, mode `s3-presigned` |
...
5. Bun/Hono API Lambda Container
6. DynamoDB `ClipForgeTable`
7. S3 Original Videos Bucket
8. S3 Derived Assets Bucket
9. Video CloudFront Distribution
10. SQS Processing Queue
11. SQS Dead Letter Queue
12. ECR/CDK Asset Repository

Use these arrows:

- Browser -> Frontend CloudFront -> S3 frontend bucket
- Browser -> Lambda Function URL -> API Lambda
- API Lambda -> DynamoDB
